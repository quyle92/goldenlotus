...
?>
	...
	<div class="row">
          	<div class="col-md-2" style="margin-bottom:5px">Bác sĩ:</div>
            <div class="col-md-3" style="margin-bottom:5px">
				<select name="bacsi">
					<option value="all">Tat ca</option>
<?php
...
?>
            ...
            <div class="col-md-2" style="margin-bottom:5px">CSKH: </div>
            <div class="col-md-3" style="margin-bottom:5px">
				<select name="cskh">
					<option value="all">Tat ca</option>
<?php
...
<?php 		
	...
	//----loc theo bac si -----//
	if($mabacsi != "" && $mabacsi != "all")
		$sql2 = $sql2 . " and a.MaBacSi like '$mabacsi'";
	
	//----loc theo trung tam -----//
	if($macskh != "" && $macskh != "all")
		$sql2 = $sql2 . " and a.MaNV like '$macskh'";
	...
	//
	//
	$sql2="select * from (select ROW_NUMBER() OVER (ORDER BY DATEDIFF(DAY,GioBatDau,GETDATE()),DATEDIFF(HOUR,GioBatDau,GETDATE()), DATEDIFF(MINUTE,GioBatDau,GETDATE())) AS RowId, a.*, b.TenNV as TenBacSi, c.TenNV as TenNV, d.TenTrungTam, e.Facebook, e.Zalo from tblKhachHangBooking a 
		left join tblDMNhanVien b on a.MaBacSi = b.MaNV 
		left join tblDMNhanVien c on a.MaNV = c.MaNV 
		left join tblDMTrungTam d on a.MaTrungTam = d.MaTrungTam 
		left join tblDMKHNCC e on a.MaKH = e.MaDoiTuong Where 1=1"; 
...
//	echo $sql2;
	...
	?>      
        <tr class="success">
          ...
          <td><?php $date2 = date_create($r2['GioKetThuc']); echo date_format($date2,'d-m-Y H:i:s'); ?></td>
          <td><?php echo $r2['TenBacSi'];?></td>
