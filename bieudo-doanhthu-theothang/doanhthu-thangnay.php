...
$this_month = date('yy/m');
  ...
  for ( $i = 1; $i <= 31; $i++ ){
    ...
    if($i <= 9){
     $sql .= "SUM(CASE WHEN substring(Convert(varchar,GioVao,111),0,11) like '" . $this_month . "/0" . $i ."' Then TienThucTra  Else 0 END) as DoanhThu_0" . $i . ", "; 
   }

    if($i > 9){
      $sql .= "SUM(CASE WHEN substring(Convert(varchar,GioVao,111),0,11) like '" . $this_month . "/" . $i ."' Then TienThucTra  Else 0 END) as DoanhThu_" . $i . ", "; 
    }
  }
